...
const formatJobToEvent = (job) => ({
  id: job.id,
  title: `${job.customers?.name || 'Khách hàng không xác định'} - ${job.job_description}`,
  start: new Date(job.scheduled_date),
  end: new Date(job.check_out_time || job.scheduled_date),
...
export default function ScheduleCalendar() {
  ...
  const fetchJobs = useCallback(async () => {
    setLoading(true);
    const { data, error } = await supabase.from('jobs').select(`*, customers ( name )`);
    if (error) {
