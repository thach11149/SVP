...
export default function CustomerForm({ open, onClose, onSave, customerToEdit }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let savedData;
      // Nếu có customerToEdit, nghĩa là đang ở chế độ "Sửa"
      if (customerToEdit) {
      ...
      onSave(savedData); // Gọi callback để cập nhật UI, truyền vào dữ liệu đã lưu
      onClose(); // Đóng dialog
    } catch (error) {
